...
- **`.design-sync/ds-entry.ts` is the bundle entry** (`cfg.entry`) — a barrel
  written purely for the sync. **Adding a component to `src/shared/ui/` means
...
`src/shared/ui/` into `dist/types/`, which the converter finds ahead of the
...
- **The barrel goes stale silently.** A component added to `src/shared/ui/` but not
  to `.design-sync/ds-entry.ts` simply never appears; nothing errors. Diff the
  barrel against `src/shared/ui/` on every sync.
